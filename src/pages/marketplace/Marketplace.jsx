...
import { Search, SlidersHorizontal, Clock, Star, ChevronDown, Flame, Filter, X } from 'lucide-react';
...
const Marketplace = () => {
  ...
  const canonicalFilters = useMemo(() => {
    ...
    const trendingGroup = searchParams.get('trending') || searchParams.get('trendingGroup') || '';
  ...
  // 2. Fetch Taxonomy Metadata once
  useEffect(() => {
    setCategories(marketplaceService.getCategories());
    setTrendingGroups(marketplaceService.getTrendingGroups());
    setTags(marketplaceService.getTags());
  }, []);

  // 3. Dynamic Subcategories based on active category filter
  useEffect(() => {
    if (canonicalFilters.category) {
      setSubcategories(marketplaceService.getSubcategories(canonicalFilters.category));
    } else {
      setSubcategories([]);
    }
  }, [canonicalFilters.category]);

  // 4. Query services via single source of truth (marketplaceService.filterServices)
  useEffect(() => {
    setLoading(true);
    const result = marketplaceService.filterServices(canonicalFilters);
    setServices(result.services || result);
    setTotal(result.total || (result.services || result).length);
    setLoading(false);
  }, [canonicalFilters]);
  ...
  const updateFilters = (newFilters) => {
    ...
    // Handle route navigation if category / subcategory changed
  ...
  const handleLoadMore = () => {
    ...
    const result = marketplaceService.filterServices({ ...canonicalFilters, page: nextPage });
    setServices(prev => [...prev, ...(result.services || result)]);
    updateFilters({ ...canonicalFilters, page: nextPage });
  };

  // Derive active taxonomy labels for Breadcrumbs & Title
  const activeCategoryObj = useMemo(() => {
    return marketplaceService.getCategoryBySlug(canonicalFilters.category);
  }, [canonicalFilters.category]);

  const activeSubcategoryObj = useMemo(() => {
    return marketplaceService.getSubcategoryBySlug(canonicalFilters.category, canonicalFilters.subcategory);
  }, [canonicalFilters.category, canonicalFilters.subcategory]);
  ...
  const pageDescription = activeSubcategoryObj?.description || activeCategoryObj?.description || activeTrendingObj?.description || 'Discover top vetted freelancers and high-performing services tailored for your business needs.';
  ...
  return (
    <div style={{ padding: 'var(--space-xl) 0 var(--space-3xl)' }}>
      <div className="container">
        ...
        <div style={{ marginBottom: 'var(--space-xl)' }}>
          ...
          <p style={{ color: 'var(--color-text-light)', fontSize: 'var(--text-sm)' }}>
            {loading ? 'Querying marketplace...' : `${total} service${total !== 1 ? 's' : ''} available`}
          </p>
        </div>
            ...
            {loading ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 'var(--space-md)' }}>
                {Array.from({ length: 8 }).map((_, i) => (
                  <div key={i} style={{ border: '1px solid rgba(255,255,255,0.06)', borderRadius: 12, overflow: 'hidden', background: 'rgba(15,23,42,0.4)', animation: 'pulse 1.5s ease infinite', height: '280px' }} />
                ))}
              </div>
            ) : services.length > 0 ? (
              <>
                ...
                {services.length < total && (
                  <div style={{ textAlign: 'center', marginTop: 48 }}>
                    ...
                    >
                      <ChevronDown size={16} /> Load More ({total - services.length} remaining)
                    </button>
                  </div>
                )}
              </>
              ...
              <div style={{
                ...
                <p style={{ marginBottom: 'var(--space-md)', color: 'var(--color-text-muted)' }}>Try resetting filters or adjusting search keywords.</p>
      ...
      {/* Responsive Styles & Mobile Filter Modal Drawer */}
      <style>{`
